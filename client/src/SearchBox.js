...
class SearchBox extends Component {
    ...
    _getOptions( src, keyName, moreResultsTarget ) {
        let options = [];
        ...
        if ( this.state.limit[moreResultsTarget] === this.props.limit ) {
            options.push((
                <Option disabled key={keyName} className="show-all">
                    <a onClick={() => { this._handleMoreResults( moreResultsTarget ); }}>
                        <Icon type="down" />
                    </a>
                </Option>));
        } else {
            options.push((
                <Option disabled key={keyName} className="show-all">
                    <Col span={12}>
                        <a onClick={() => { this._handleLessResults( moreResultsTarget ); }}>
                            <Icon type="up" />
                        </a>
                    </Col>
                    <Col span={12}>
    ...
    render() {
        let dataSource = [
            (<OptGroup label="Keywords">
                {this._getOptions(this.state.dataSource.keywords, "moreKeywords", "keywords")}
            </OptGroup>),
            (<OptGroup label="Keyphrases">
                {this._getOptions(this.state.dataSource.keyphrases, "moreKeyphrases", "keyphrases")}
            </OptGroup>)
        ];
